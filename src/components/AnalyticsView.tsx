...
import { 
  BarChart2, Clock, CheckCircle, TrendingUp, AlertTriangle, 
  BookOpen, Sparkles, ChevronRight, MapPin, Calendar 
} from 'lucide-react';
...
export default function AnalyticsView({ progress, onNavigateToQuiz }: AnalyticsViewProps) {
  // Mock monthly performance indicators
  const WEEKLY_TREND = [
    { day: 'Mon', count: 3, correct: 2 },
    { day: 'Tue', count: 5, correct: 4 },
    { day: 'Wed', count: 0, correct: 0 },
    { day: 'Thu', count: 8, correct: 6 },
    { day: 'Fri', count: 12, correct: 10 },
    { day: 'Sat', count: 4, correct: 3 },
    { day: 'Sun', count: 6, correct: 5 }
  ];

  // Subject recommendations
  const SUBJECTS_STRENGTH = [
    { name: 'Mathematics', xp: progress.subjectsStudied['Mathematics'] || 120, pct: 90, status: 'Strongest' },
    { name: 'English Language', xp: progress.subjectsStudied['English Language'] || 80, pct: 85, status: 'Strong' },
    { name: 'Physics', xp: progress.subjectsStudied['Physics'] || 40, pct: 72, status: 'Needs Revision' },
    { name: 'Chemistry', xp: progress.subjectsStudied['Chemistry'] || 20, pct: 55, status: 'Weak' }
  ];

  // Grid list of study days for activity calendar heatmap
  const HEATMAP_DAYS = Array.from({ length: 28 }, (_, i) => {
    const activeDays = [2, 3, 5, 8, 9, 10, 11, 14, 15, 17, 18, 22, 23, 24, 25, 27];
    return { day: i + 1, active: activeDays.includes(i + 1) };
  });

  return (
    <div id="analytics-view-root" className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-100 p-5 rounded-2xl relative">
            <span className="text-3xs font-extrabold uppercase tracking-widest text-slate-400 block">Overall Accuracy</span>
            <span className="font-mono text-2xl font-black text-indigo-700 block mt-1">{progress.accuracy}%</span>
            <div className="h-1 bg-indigo-100 rounded-full mt-2.5 overflow-hidden">
              <div className="h-full bg-indigo-600 rounded-full" style={{ width: `${progress.accuracy}%` }}></div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 p-5 rounded-2xl relative">
            <span className="text-3xs font-extrabold uppercase tracking-widest text-slate-400 block">Daily Streak</span>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
          <div className="flex justify-between items-center pb-2 border-b border-slate-100">
            <div>
              <h4 className="font-display font-bold text-slate-950 text-base flex items-center gap-1.5">
                <TrendingUp className="text-indigo-600 w-5 h-5" /> Weekly Diagnostic Performance
              </h4>
              <p className="text-2xs text-slate-500">Comparing total answered vs correct answers across the week</p>
            </div>
          </div>
          ...
          <div className="h-56 flex items-end justify-between pt-4 gap-2">
            {WEEKLY_TREND.map((t, idx) => {
              ...
              const correctHeight = t.correct > 0 ? (t.correct / maxVal) * 100 : 0;
              
              return (
                <div key={idx} className="flex flex-col items-center grow h-full justify-end group">
                  <div className="w-full max-w-[32px] h-full flex items-end gap-1 px-1 relative">
                    ...
                    <div 
                      ...
                      style={{ height: `${answeredHeight}%`, minHeight: t.count > 0 ? '4px' : '0' }}
                    ></div>
                    {/* Correct Answers bar */}
                    <div 
                      className="bg-indigo-600 rounded-t-lg grow transition group-hover:bg-indigo-700"
                      style={{ height: `${correctHeight}%`, minHeight: t.correct > 0 ? '4px' : '0' }}
                    ></div>
                  </div>
                  <span className="text-[10px] text-slate-400 font-bold mt-2 font-display">{t.day}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
          <div>
            <h4 className="font-display font-bold text-slate-950 text-base flex items-center gap-1.5">
              <Calendar className="text-indigo-600 w-5 h-5" /> Candidate Activity Heatmap
            </h4>
            <p className="text-2xs text-slate-500">Track consistency daily. Darker cards indicate completed CBT practice exam sessions</p>
          </div>
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl space-y-4">
          <h3 className="font-display font-bold text-slate-950 text-base">Subject Diagnostic Rating</h3>
              ...
              return (
                <div key={idx} className="space-y-1">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className={`font-black text-2xs uppercase ${isWeak ? 'text-red-500' : 'text-emerald-600'}`}>
                      {sub.status}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-2xs font-extrabold text-slate-500 w-8 text-right">{sub.pct}%</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="bg-indigo-950 text-white rounded-3xl p-6 shadow-xl space-y-4">
          <div className="flex gap-2.5 items-center">
            ...
            <h3 className="font-display font-bold text-white text-base">AI Prep Suggestions</h3>
          </div>

          <p className="text-xs text-slate-300 leading-relaxed">
            Excellent job in set theory, Mathematics is your sharpest skill! However, your Chemistry scores are slightly lower.
          </p>
          ...
          <div className="bg-slate-900 p-3.5 border border-slate-800 rounded-2xl flex items-start gap-2 text-2xs text-indigo-200">
            ...
            <div>
              ...
              <p className="text-slate-400 mt-0.5 leading-normal">Solve Gas Law equations in the Chemistry module to elevate your diagnostic level.</p>
            </div>
          </div>
