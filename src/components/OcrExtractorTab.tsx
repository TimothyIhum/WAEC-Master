...
export default function OcrExtractorTab({
  ...
  const [ocrExamNameHint, setOcrExamNameHint] = useState<"WAEC" | "JAMB">(
    "WAEC",
  );
  ...
  const detectDuplicate = (
    newText: string,
  ): { isDuplicate: boolean; similarQuestion?: Question } => {
  ...
  const addFilesToQueue = (files: File[]) => {
    ...
    const newQueued: QueuedFile[] = files.map((file) => {
      ...
      addLog(
        `File loaded: Name="${file.name}", Size=${formatBytes(file.size)}, MIME="${file.type}", DetectedType=${typeLabel}`,
      );
  ...
  const processQueueSequentially = async () => {
    addLog(
      `"Trigger OCR" clicked. Core scanning pipeline started with subject hint="${ocrSubjectHint}".`,
    );
    setGlobalError(null);
    ...
    if (pending.length === 0) {
      ...
      alert(
        "My CBT content manager, please select or drop some screenshots, PDFs or exam question sheets first!",
      );
      return;
    }
    ...
    for (const fileItem of pending) {
      ...
      const steps = [
        {
          progress: 25,
          log: "Analyzing layout segmentation & column structures...",
        },
        {
          progress: 45,
          log: "Applying optical character recognition (OCR) and correcting scan artifacts...",
        },
        {
          progress: 70,
          log: "Running NLP parser to segment questions, indices, and choices...",
        },
        {
          progress: 90,
          log: "Generating syllabus explanation mapping and difficulty scoring...",
        },
      ];
      ...
      try {
        ...
        if (fileItem.rawFile) {
          try {
            addLog(
              `Converting raw file "${fileItem.name}" of size ${formatBytes(fileItem.rawFile.size)} into base64 payload...`,
            );
            ...
            addLog(
              `Base64 generated successfully (${Math.round(base64Img.length / 1024)} KB).`,
            );
          } catch (readErr: any) {
            addLog(
              `[WARNING] Failed to read file as data URL, falling back. Error: ${readErr.message}`,
            );
            console.error(
              "Failed to read file as data URL, falling back.",
              readErr,
            );
          }
        }
        ...
        addLog(
          `Despatching api payload to endpoint "/api/gemini/extract-questions" for file "${fileItem.name}"...`,
        );
        ...
        addLog(
          `API Network Connection established. Response status received: ${resp.status} (${resp.statusText})`,
        );
        ...
        const parsedQs: ParsedOcrQuestion[] = (data.questions || []).map(
          (q: any, index: number) => {
            ...
            return {
              ...
              explanation:
                q.explanation || "Constructed with AI learning context.",
        ...
        addLog(
          `Successfully segmented and parsed ${parsedQs.length} exam questions from stream response.`,
        );
        ...
        addLog(
          `[CRITICAL EXCEPTION] Processing sequence halted for "${fileItem.name}". Details: ${err.message}`,
        );
        console.error("OCR pipeline failed for file:", fileItem.name, err);
        setGlobalError(
          `Failed to process page [${fileItem.name}]: ${err.message}. If this is a proxy, token limit or CORS block, please click the "Bypass Network Log & Simulate" option.`,
        );
        setFilesQueue((prev) =>
          prev.map((f) =>
            f.id === fileItem.id
  ...
  const processQueueSimulation = async () => {
    addLog(
      `[Simulation / Offline Mode] Triggered bypass simulation for subject="${ocrSubjectHint}".`,
    );
    setGlobalError(null);
    const pending = filesQueue.filter((f) => f.status === "pending");
    if (pending.length === 0) {
      ...
      alert(
        "Please add a file to the queue first, then trigger offline bypass simulation.",
      );
      return;
    }
        ...
        simulatedList.push(
          {
            ...
            correctAnswer: "1", // 156
            explanation:
              "Sum of AP is given by S_n = n/2 [2a + (n-1)d]. Here a=2, d=2, n=12. S_12 = 12/2 * [4 + 11*2] = 6 * [26] = 156.",
          ...
          },
          {
            ...
            correctAnswer: "0", // 4
            explanation:
              "Expand: 3x + 15 - 2x + 2 = 21 => x + 17 = 21 => x = 4.",
          ...
          },
          {
            ...
            explanation:
              "In a 3-4-5 right triangle, sin(A) = opposite/hypotenuse = 3/5. Thus adjacent = 4. cos(A) = adjacent/hypotenuse = 4/5. tan(A) = opposite/adjacent = 3/4. Sum = 4/5 + 3/4 = (16+15)/20 = 31/20.",
        ...
        simulatedList.push(
          {
            ...
            explanation:
              "Summing the electrons: 2+2+6+2+5 = 17. The element with atomic number 17 is Chlorine (Cl).",
          ...
          },
          {
            ...
            explanation:
              "Quantity of electricity Q = I * t = 1.5 * (2 * 3600) = 10800 C. Cu^2+ + 2e- -> Cu. Hence, 2 * 96500 C deposits 64g of Cu. 10800 C deposits (10800 * 64) / (2 * 96500) = 691200 / 193000 = 3.58 g.",
        ...
        simulatedList.push(
          {
            ...
            explanation:
              "The word 'gregarious' means fond of company and highly sociable. The opposite of being social is being solitary or reclusive.",
          ...
          },
          {
            ...
            explanation:
              "The gerund phrase 'Writing clean system code' is the noun-form acting as the subject of the predicate verb 'is'.",
        ...
        simulatedList.push(
          {
            ...
            explanation:
              "Pressure is calculated as force divided by perpendicular area: P = F / A = 200 / 0.5 = 400 Pascals (Pa).",
          ...
          },
          {
            ...
            correctAnswer: "2", // 40x
            explanation:
              "Approximated microscope magnification is M = (L_tube / f_obj) * (D_least / f_eye). Standard D_least = 25 cm. M = (20 / 2.0) * (25 / 5.0) = 10 * 5 = 50x (or using general simple M = L / f_obj * D / f_eye etc. here tube magnification is 10 and eyepiece magnification is 4. Thus total magnification is 10 * 4 = 40x).",
          ...
          },
          {
            ...
            explanation:
              "Relativistic quantum mechanics unifies relativity (high speeds near speed of light) with quantum theory (subatomic quantum states) to correctly predict behavior of fast subatomic particles.",
  ...
  const updateParsedQuestionField = (
    id: string,
    field: keyof ParsedOcrQuestion,
    value: any,
  ) => {
  ...
  const approveAndCommitQuestion = (ocrQ: ParsedOcrQuestion) => {
    ...
    const qYear =
      ocrQ.detectedYear !== undefined ? ocrQ.detectedYear : Number(ocrYearHint);
    const safeSubjectSlug = qSubject
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "");
    const paperId = `${ocrExamNameHint.toLowerCase()}-${safeSubjectSlug}-${qYear}`;
    const paperTitle = `${ocrExamNameHint} ${qSubject} ${qYear}`;
    ...
    const formatted: Question = {
      ...
      questionNumber: ocrQ.question_number,
      paperId,
      paperTitle,
      sourceFileName: ocrQ.originalFileName,
      topic: ocrQ.topic,
      type: ocrQ.type,
      text: ocrQ.text,
      ...
      marks:
        ocrQ.difficulty === "Hard" ? 5 : ocrQ.difficulty === "Medium" ? 3 : 2,
    };
  ...
  const approveAllNonDuplicates = () => {
    ...
    alert(
      `💡 Successfully batch-approved and committed ${approvedCount} WAEC CBT questions into the active system store!`,
    );
  };
  ...
  const clearCompletedRoster = () => {
    if (
      confirm("Discard all extracted questions from review list completely?")
    ) {
  ...
  return (
    <div
      id="ocr-extractor-wrapper"
      className="space-y-6 pt-4 animate-fadeIn text-xs text-slate-700"
    >
      {/* HEADER OVERVIEW */}
      <div className="bg-slate-50 border border-slate-150 p-5 rounded-2xl flex flex-col md:flex-row justify-between gap-4">
        <div className="space-y-1 md:max-w-xl">
          ...
          <p className="text-[11px] leading-relaxed text-slate-500">
            Automate CBT curation from scanned past paper booklets, PDFs,
            photos, or desktop screenshots. The system applies professional OCR
            character reconstruction, detects subjects, question numbering
            structures, choices, correct keys, auto-composes step-by-step
            explanations, flags duplicates, and classifies modules directly.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 shrink-0 md:w-56 text-center">
          <div className="bg-white p-2.5 rounded-xl border border-slate-150">
            <span className="font-mono text-[10px] text-slate-400 block tracking-wider uppercase leading-none">
              Scanned Sheets
            </span>
          ...
          <div className="bg-white p-2.5 rounded-xl border border-slate-150">
            <span className="font-mono text-[10px] text-emerald-500 block tracking-wider uppercase leading-none">
              Processed OK
            </span>
          ...
          <div className="bg-white p-2.5 rounded-xl border border-slate-150">
            <span className="font-mono text-[10px] text-red-400 block tracking-wider uppercase leading-none">
              Errors/Fails
            </span>
          ...
          <div className="bg-white p-2.5 rounded-xl border border-indigo-100 bg-indigo-50/20">
            <span className="font-mono text-[10px] text-indigo-600 block tracking-wider uppercase leading-none">
              Duplicates
            </span>
      ...
      {globalError && (
        <div
          id="ocr-global-error-panel"
          className="bg-red-50 border-l-4 border-l-red-500 border border-red-200 text-red-700 p-4 rounded-xl space-y-2 animate-fadeIn"
        >
          <div className="flex gap-2.5 items-start">
            <AlertTriangle className="w-5 h-5 shrink-0 text-red-600 mt-0.5" />
            <div>
              <p className="font-extrabold text-slate-800 text-xs">
                CBT Content Engine Pipeline Warning
              </p>
          ...
          <div className="flex gap-2.5 pl-7.5">
            ...
            >
              <Sparkles className="w-3.5 h-3.5" /> Bypass Network Log & Force
              Parse Offline (Syllabus Seed)
            </button>
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-white border border-slate-150 rounded-2xl p-4.5 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <span className="text-[10px] text-slate-450 mt-1 block">
                Help AI map dynamic context keywords accurately.
              </span>
            </div>
            ...
            <div className="space-y-1">
              ...
              <span className="text-[10px] text-slate-450 mt-1 block">
                Specify which year these questions map to.
              </span>
            </div>
            ...
            <div className="space-y-1">
              ...
              <span className="text-[10px] text-slate-450 mt-1 block">
                Determines simulator body categorization.
              </span>
            </div>
            ...
            >
              ...
              />
              <Upload
                className={`w-8 h-8 mx-auto mb-2 transition ${isDragging ? "text-indigo-600 scale-110" : "text-slate-400 group-hover:text-indigo-600"}`}
              />
              <span
                className={`font-bold block text-[11px] transition ${isDragging ? "text-indigo-700" : "text-slate-700 group-hover:text-indigo-700"}`}
              >
                {isDragging
              ...
              <p className="text-[9px] text-slate-400 mt-1">
                Supports screenshots, photos, high-res question scans & PDFs
              </p>
            </div>
            ...
            >
              ...
              <div>
                <span className="font-bold text-slate-700 block text-[10px] text-left group-hover:text-amber-600">
                  Bulk Import ZIP Past Papers
                </span>
                <p className="text-[9px] text-slate-400 text-left leading-none mt-0.5">
                  Extract hundreds of pages recursively
                </p>
              </div>
            </div>
          </div>
            ...
            {filesQueue.length === 0 ? (
              ...
              <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
                {filesQueue.map((file) => (
                  <div
                    key={file.id}
                    className="bg-slate-50 p-2.5 rounded-xl border border-slate-150 flex flex-col gap-1.5"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 overflow-hidden">
                        <FileText className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                        <span className="font-bold text-slate-800 truncate block max-w-40">
                          {file.name}
                        </span>
                      </div>
                ...
                <button
                  ...
                  disabled={
                    ocrLoading ||
                    filesQueue.filter((f) => f.status === "pending").length ===
                      0
                  }
                ...
                >
                  {ocrLoading ? (
                    ...
                    <>
                      <Sparkles className="w-4 h-4" /> Trigger OCR Question
                      segmenter
                    </>
                  )}
                </button>
          ...
          {showDiagnostics && (
            <div
              id="ocr-diagnosis-console"
              className="bg-slate-900 border border-slate-800 rounded-2xl p-4.5 text-[10px] font-mono text-slate-300 space-y-3 shadow-md animate-fadeIn"
            >
              ...
              {diagnosticLogs.length === 0 ? (
                <p className="text-slate-500 italic py-2 text-center text-[10px]">
                  Waiting for events... Queue up exam past papers and click
                  "Trigger OCR", or tap the direct offline bypass trigger below
                  if you have proxy network blockages.
                </p>
              ) : (
                <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                  {diagnosticLogs.map((log, i) => (
                    <div
                      key={i}
                      className="leading-relaxed hover:bg-slate-850/50 p-0.5 rounded border-b border-slate-850/20"
                    >
                      <span className="text-slate-550 select-none mr-1">▶</span>{" "}
                      {log}
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-2 border-t border-slate-850">
                ...
                >
                  <Sparkles className="w-3.5 h-3.5 text-emerald-300" /> Bypass
                  Connection Wait & Simulate Parse
                </button>
                <span className="text-[9px] text-slate-500 text-center block mt-1">
                  Instant local parsing bypass to overcome sandbox/CORS limits.
                </span>
              </div>
            </div>
          )}
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-100 pb-2.5 mb-4 gap-2">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    Edit OCR errors, verify correct options, verify diagrams,
                    and commit questions
                  </p>
                </div>
                  ...
                  {extractedQuestions.length > 0 && (
                    <>
                      ...
                      >
                        ...
                        {
                          extractedQuestions.filter((q) => !q.isDuplicate)
                            .length
                        }
                        )
                      </button>
                ...
                <div className="h-64 flex flex-col items-center justify-center text-slate-400 text-center gap-2">
                  ...
                  <div>
                    <span className="font-bold text-slate-700">
                      Review Board is Empty
                    </span>
                    <p className="text-[10px] max-w-xs mt-1 leading-relaxed">
                      Upload and trigger the OCR process on CBT question papers.
                      Parsed candidate items will populate here for curation,
                      metadata fixes, and fast system commit.
                    </p>
                  </div>
                </div>
              ) : (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2.5 pb-2 border-b border-dashed border-slate-200/70 text-[10px]">
                          <div className="flex items-center gap-2.5">
                            ...
                            <span
                              className="bg-slate-250 text-slate-600 font-extrabold px-1.5 py-0.5 rounded-sm uppercase text-[8px] max-w-[140px] truncate"
                              title={ocrQ.originalFileName}
                            >
                          ...
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-[9px] text-slate-450 uppercase">
                              OCR Confidence:
                            </span>
                            <span
                              className={`font-mono font-black text-xs px-1.5 py-0.5 rounded-sm ${
                                ocrQ.confidence >= 90
                        ...
                        {isEditing ? (
                          <div className="space-y-3 pt-1 animate-fadeIn">
                            {/* Question text */}
                            <div className="space-y-1">
                              <label className="font-extrabold text-slate-500 uppercase text-[9px]">
                                Question Prompt
                              </label>
                              <textarea
                                value={ocrQ.text}
                                onChange={(e) =>
                                  updateParsedQuestionField(
                                    ocrQ.id,
                                    "text",
                                    e.target.value,
                                  )
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                              {ocrQ.options.map((opt, oIdx) => (
                                <div key={oIdx} className="space-y-0.5">
                                  <label className="font-extrabold text-slate-450 uppercase text-[8px]">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              <div className="space-y-1">
                                <label className="font-extrabold text-slate-500 uppercase text-[9px]">
                                  Step-by-step Explanation Solver
                                </label>
                                <textarea
                          ...
                          <div className="space-y-2.5">
                            <p className="font-black text-slate-800 text-xs leading-relaxed">
                              {ocrQ.text}
                            </p>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                              {ocrQ.options.map((opt, oIdx) => {
                                const isCorrect =
                                  String(oIdx) === ocrQ.correctAnswer;
                                return (
                                  <div
                                    key={oIdx}
