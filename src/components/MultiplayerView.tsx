...
import { 
  Users, Flame, Award, Timer, RefreshCw, Send, CheckCircle, 
  XCircle, Zap, Smile, Trophy, Bot, Play, ShieldAlert 
} from 'lucide-react';
...
export default function MultiplayerView({ currentUsername, avatar, level, onBattleWon }: MultiplayerViewProps) {
  // Phase states: 'init' -> 'connecting' -> 'fight' -> 'winner'
  const [phase, setPhase] = useState<'init' | 'connecting' | 'fight' | 'winner'>('init');
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (data.status === 'matched') {
          ...
          // Pull questions matching subject and shuffle them to avoid repetition
          const filteredQs = SAMPLE_QUESTIONS.filter(q => q.subject === selectedSubject);
          const shuffledQs = [...filteredQs].sort(() => 0.5 - Math.random());
          const matchedQs = shuffledQs.slice(0, 5);
          setQuestions(matchedQs.length > 0 ? matchedQs : SAMPLE_QUESTIONS.slice(0, 5));
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Track reactions
          const opponent = currentRoom.player1.username === currentUsername 
            ? currentRoom.player2 
            : currentRoom.player1;
          ...
          if (currentRoom.status === 'finished') {
            ...
            const localPlayer = currentRoom.player1.username === currentUsername ? currentRoom.player1 : currentRoom.player2;
            const oppPlayer = currentRoom.player1.username === currentUsername ? currentRoom.player2 : currentRoom.player1;

            if (localPlayer && oppPlayer && localPlayer.score > oppPlayer.score) {
  ...
  const handleAnswerSelection = async (optIdxStr: string) => {
    ...
    if (currentQ && currentQ.correctAnswer.toLowerCase() === optIdxStr.toLowerCase()) {
      isCorrect = true;
  ...
  const getOpponentUser = () => {
    if (!room) return null;
    return room.player1.username === currentUsername ? room.player2 : room.player1;
  };

  const getLocalUser = () => {
    if (!room) return null;
    return room.player1.username === currentUsername ? room.player1 : room.player2;
  };

  return (
    <div id="multiplayer-root" className="bg-white rounded-3xl border border-slate-100 p-6 md:p-8 shadow-xl max-w-4xl mx-auto space-y-8">
        ...
        <div id="multiplayer-init-view" className="text-center space-y-8 py-6">
          <div className="max-w-md mx-auto space-y-3">
            ...
            <p className="text-sm text-slate-500">
              Challenge other candidates in real-time speed assessments. Answer correctly, react with emojis, and steal XP trophies!
            </p>
          </div>

          <div className="max-w-sm mx-auto space-y-4">
            <div className="space-y-1 text-left">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Choose Battle Subject</label>
      ...
      {phase === 'connecting' && (
        <div id="multiplayer-queue-view" className="text-center py-12 space-y-8">
          ...
          <div className="space-y-2">
            <h3 className="font-display font-bold text-xl text-slate-900">Searching for Opponents...</h3>
            <p className="text-xs text-slate-500 max-w-xs mx-auto">
              Querying active candidate queues on the server. If matchmaking exceeds 5 seconds, an advanced AI CBT bot will spawn automatically!
            </p>
          </div>
          ...
          <div className="inline-flex items-center gap-2 bg-slate-50 border border-slate-100 py-1.5 px-4 rounded-full text-xs text-slate-600">
            <Bot className="w-4 h-4 text-indigo-500 shrink-0" /> Bot fallback initialized. Ready to play!
          </div>
        </div>
      )}
        ...
        <div id="multiplayer-fight-panel" className="space-y-6">
          ...
          <div className="grid grid-cols-11 items-center gap-2 bg-slate-950 text-white p-4.5 rounded-2xl border border-slate-900 relative">
            ...
            <div className="col-span-4 text-center space-y-1">
              <span className="text-2xl block">{getLocalUser()?.avatar || '👤'}</span>
              <h4 className="font-extrabold text-xs text-white truncate">{getLocalUser()?.username}</h4>
              <span className="font-mono text-xl font-bold text-emerald-400 block">{getLocalUser()?.score || 0} pts</span>
              <span className="text-[10px] text-slate-500">Q {getLocalUser()?.currentQuestionIndex || 0} / 5</span>
            </div>

            {/* VS Indicator */}
            <div className="col-span-3 text-center space-y-2">
              <span className="text-2xs font-extrabold tracking-widest text-indigo-500 uppercase block">VERSUS DUAL</span>
            ...
            <div className="col-span-4 text-center space-y-1 relative">
              <span className="text-2xl block">{getOpponentUser() ? getOpponentUser()?.avatar : '⏳'}</span>
              <h4 className="font-extrabold text-xs text-white truncate">
                {getOpponentUser() ? getOpponentUser()?.username : 'Connecting...'}
              </h4>
              <span className="font-mono text-xl font-bold text-cyan-400 block">
                {getOpponentUser() ? getOpponentUser()?.score : 0} pts
              </span>
              <span className="text-[10px] text-slate-500">
                Q {getOpponentUser() ? getOpponentUser()?.currentQuestionIndex : 0} / 5
              </span>
          ...
          {currentQIdx < 5 ? (
            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-6">
              <div className="flex justify-between items-center pb-2 border-b border-slate-150">
                <span className="text-xs font-black uppercase tracking-widest text-slate-400">Question {currentQIdx + 1} of 5</span>
                <span className="text-xs font-bold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-full">{questions[currentQIdx].topic}</span>
              </div>
              ...
              {/* OPTIONS */}
              {questions[currentQIdx].type === 'mcq' && questions[currentQIdx].options ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                  {questions[currentQIdx].options.map((opt, oIdx) => {
                    ...
                    return (
                      ...
                      >
                        <span className="font-bold mr-2 text-indigo-600">{ltr}.</span> {opt}
                      </button>
                    );
                  })}
                </div>
                ...
                <div className="space-y-2 text-xs">
                  <span className="text-slate-400 font-bold uppercase tracking-wider block">Written Entry</span>
                  <input
                    type="text"
                    disabled={selectedOpt !== null}
                    placeholder="Enter short answer, then press Submit"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') handleAnswerSelection((e.target as HTMLInputElement).value);
                    }}
            ...
            <div className="p-8 bg-slate-50 rounded-2xl border border-slate-100 text-center space-y-4">
              <div className="w-12 h-12 rounded-full border-2 border-dashed border-indigo-500 animate-spin mx-auto flex items-center justify-center text-xl">⏳</div>
              <h4 className="font-display font-bold text-slate-800 text-sm">Perfect Round Complete!</h4>
              <p className="text-xs text-slate-500 max-w-xs mx-auto">
                You have finished all 5 questions. We are awaiting your competitor to submit their diagnostic sheet. Hang tight!
              </p>
            </div>
          )}
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex items-center gap-3">
            <span className="text-2xs font-extrabold text-slate-400 uppercase tracking-widest">Blast emoji:</span>
      ...
      {phase === 'winner' && room && (
        <div id="multiplayer-results-panel" className="text-center py-6 space-y-6 max-w-sm mx-auto">
          {/* Winner logic */}
          {room.winner === currentUsername ? (
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <h2 className="font-display font-black text-2xl text-slate-900">Champion! Victory Dual!</h2>
                <p className="text-xs text-slate-500">You completely outclassed your competitor and secured victory.</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <h2 className="font-display font-extrabold text-xl text-slate-800">Spectacular Draw!</h2>
                <p className="text-xs text-slate-500">You scored the exact same points! Perfect matching reflexes.</p>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <h2 className="font-display font-extrabold text-xl text-slate-800 font-sans">Defeat</h2>
                <p className="text-xs text-slate-500">Your opponent excelled in diagnostic speed. Train with AI coach to strengthen your agility!</p>
              </div>
            </div>
          )}
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-150 text-xs text-slate-700 divide-y divide-slate-200">
            <div className="flex justify-between items-center py-2">
              ...
              <span className="font-mono font-bold text-indigo-700">{getLocalUser()?.score || 0} pts</span>
            </div>
            <div className="flex justify-between items-center py-2">
              <span className="font-semibold">Opponent Score</span>
              <span className="font-mono font-bold text-indigo-700">{getOpponentUser()?.score || 0} pts</span>
            </div>
          </div>
