...
interface CbtSimulatorProps {
  userXP: number;
  userLevel: number;
  onQuizCompleted: (xpEarned: number, correctCount: number, totalCount: number, timeSpentMin: number) => void;
...
type ExamMode = 'waec_simulation' | 'jamb_practice' | 'speed_challenge' | 'survival' | 'topic_mastery';
...
export default function CbtSimulator({
  ...
  questionsList = SAMPLE_QUESTIONS
}: CbtSimulatorProps) {
  ...
  const handleStartExam = () => {
    let examType: 'WAEC' | 'JAMB' = selectedMode === 'jamb_practice' ? 'JAMB' : 'WAEC';
    
    let countParam: number | 'standard' | 'official_exam' = 'official_exam';
    if (customCountSetting === 'all_available' || customCountSetting === 'official_exam') {
    ...
    let topicParam = selectedMode === 'topic_mastery' ? selectedTopic : undefined;
  ...
  const handleAnswerSelect = (indexStr: string) => {
    ...
    if (selectedMode === 'survival') {
      const isCorrect = questions[currentIdx].correctAnswer.toLowerCase() === indexStr.toLowerCase();
      if (!isCorrect) {
        setLives(l => {
          const updated = l - 1;
          if (updated <= 0) {
  ...
  return (
    <div id="cbt-simulator-root" className="h-full space-y-6">
      ...
      {phase === 'select' && (
        <div id="cbt-settings-panel" className="bg-white rounded-3xl border border-slate-100 shadow-xl p-8 space-y-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-500">Practice past WAEC papers, solve formulas, or survive high-stakes time trials</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white border-0 font-bold rounded-2xl text-xs flex items-center gap-2 shadow-lg shadow-emerald-100 transition cursor-pointer"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              <label className="text-xs font-black uppercase text-slate-500 tracking-wider">1. Select WAEC Subject</label>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                {subjectsList.map(subj => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-semibold text-xs leading-tight">{subj}</span>

                      <div className="flex justify-between items-center mt-2">
                        {isDownloaded ? (
                          <span className={`text-[9px] font-bold py-0.5 px-2 rounded-full ${selectedSubject === subj ? 'bg-indigo-500/50 text-white' : 'bg-emerald-100 text-emerald-800'}`}>
                            Offline OK
                          </span>
                        ) : (
                          <button
            ...
            <div className="space-y-4">
              <label className="text-xs font-black uppercase text-slate-500 tracking-wider">2. Choose Assessment Mode</label>
              <div className="space-y-3">
                ...
                >
                  <div className="p-3 bg-amber-100 rounded-xl text-amber-700 font-extrabold text-xs">WAEC</div>
                  <div className="grow">
                    <h4 className="font-bold text-slate-900 text-sm">WAEC CBT Examination Simulation</h4>
                    <p className="text-xs text-slate-500">Mocks real CBT conditions with strict timers and proportional marking.</p>
                  </div>
                </div>
                ...
                >
                  <div className="p-3 bg-purple-100 rounded-xl text-purple-700 font-extrabold text-xs">JAMB</div>
                  <div className="grow">
                    <h4 className="font-bold text-slate-900 text-sm">JAMB Speed Booster</h4>
                    <p className="text-xs text-slate-500">High velocity speed practice to expand your rapid answer choices.</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="grow w-0">
                    <h4 className="font-bold text-slate-900 text-sm">Lives Survival CBT</h4>
                    <p className="text-xs text-slate-500 truncate">3 Lives. Lose a life for every incorrect answer. How far can you survive?</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="grow">
                    <h4 className="font-bold text-slate-900 text-sm">Speed Sprint Round</h4>
                    <p className="text-xs text-slate-500">Exactly 30 seconds per question limit. Earn huge double XP multipliers!</p>
                  </div>
                </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="grow">
                      <h4 className="font-bold text-slate-900 text-sm">Syllabus Topic Mastery</h4>
                      <p className="text-xs text-slate-500">Focus your studies on 1 challenging syllabus segment.</p>
                    </div>
                  </div>
                  ...
                  {selectedMode === 'topic_mastery' && (
                    <div className="pl-14">
                      ...
                      >
                        {(TOPICS_BY_SUBJECT[selectedSubject] || []).map((tp, idx) => (
          ...
          <div id="cbt-custom-filters" className="pt-6 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
            {/* Year Selector card */}
            <div className="space-y-3">
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (yr === 'all' && customCountSetting === 'all_available') {
                    ...
                    >
                      {yr === 'all' ? 'All Years (Aesthetic Mix)' : `${yr} Past Edition`}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  📝 Simulated Practice Set ({selectedSubject === 'English Language' ? '60' : (selectedMode === 'jamb_practice' ? '40' : '50')} Qs)
                </button>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-50 border border-slate-100/80 p-3 rounded-2xl flex items-center justify-between gap-4">
                <div className="space-y-0.5 shrink-0">
                  <span className="text-[10px] uppercase font-black tracking-wider text-slate-400 block">Exact Minutes</span>
                  <div className="flex items-center gap-1">
                    <input 
                      type="number"
                      min="5"
                      max="300"
                      value={customTimeSetting === 'default' ? 45 : customTimeSetting}
                      onChange={(e) => {
                ...
                <div className="grow">
                  <input 
                    ...
                    step="5"
                    value={customTimeSetting === 'default' ? 45 : customTimeSetting}
                    onChange={(e) => setCustomTimeSetting(parseInt(e.target.value))}
          ...
          >
            Launch {selectedSubject} Preparation <Play className="w-5 h-5 fill-white" />
          </button>
        </div>
      )}
      ...
      {phase === 'quiz' && questions.length > 0 && (
        <div id="cbt-active-exam-panel" className="bg-slate-900 text-white rounded-3xl p-6 md:p-8 shadow-2xl space-y-6 relative border border-slate-800">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-start gap-3">
              <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">{questions[currentIdx].topic}</span>
              <button
                type="button"
            ...
            {questions[currentIdx].diagramUrl && (
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-4 flex justify-center py-6 text-slate-500 text-xs">
                📸 WAEC Figure 1.3 - Chemical Cell Electromotive diagram placeholder
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-2 mt-4 pt-3.5 border-t border-slate-800 text-2xs text-slate-400 font-mono">
              ...
              <span>•</span>
              <span>Academic Year: <strong className="text-slate-200">{questions[currentIdx].examYear || 2021}</strong></span>
              <span>•</span>
              <span>Question Number: <strong className="text-slate-200">No. {questions[currentIdx].questionNumber || 12}</strong></span>
            </div>
          </div>
            ...
            {questions[currentIdx].type === 'mcq' && questions[currentIdx].options ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                {questions[currentIdx].options.map((opt, optIdx) => {
                  ...
                  const isSelected = userAnswers[questions[currentIdx].id] === optIdx.toString();
                  return (
                    <button
                      key={optIdx}
                    ...
                    >
                      <span className={`w-7 h-7 rounded-lg font-bold flex items-center justify-center text-xs shrink-0 ${isSelected ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
                        {letter}
                      </span>
                      <span>{opt}</span>
                    </button>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400">Type Your Short Answer Below</label>
                <input 
                  type="text"
                  placeholder="e.g. 30"
          ...
          {showHint && (
            <div className="bg-indigo-950/50 border border-indigo-900 text-indigo-200 rounded-2xl p-4 text-xs font-semibold animate-fadeIn">
              💡 Hint: {questions[currentIdx].hint || "Try splitting your equations step-by-step or converting all factors."}
            </div>
          )}
          ...
          <div className="flex justify-between items-center pt-6 border-t border-slate-800 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {currentIdx === questions.length - 1 ? 'Submit CBT Exam' : 'Next'}
      ...
      {phase === 'review' && (
        <div id="cbt-review-panel" className="bg-white rounded-3xl p-8 border border-slate-100 shadow-xl space-y-8">
          {/* Trophy Header info */}
          <div className="text-center space-y-4 max-w-sm mx-auto pb-4 border-b border-slate-100">
            ...
            <div className="space-y-1">
              <h2 className="font-display font-black text-2xl text-slate-900">Quiz Completed!</h2>
              <p className="text-xs text-slate-500">Excellent effort on {selectedSubject} in {selectedMode.toUpperCase().replace('_', ' ')} mode</p>
            </div>

            <div className="flex justify-center gap-6 py-2">
              <div className="text-center">
                <span className="text-slate-400 text-2xs font-extrabold uppercase tracking-widest block">Accuracy</span>
                <span className="font-mono text-2xl font-black text-indigo-600">{getPercentageString()}</span>
              </div>
              <div className="text-center">
                <span className="text-slate-400 text-2xs font-extrabold uppercase tracking-widest block">XP Gained</span>
                <span className="font-mono text-2xl font-black text-emerald-600">+{calculatedXp} <span className="text-xs">XP</span></span>
              </div>
            </div>
          </div>
          ...
          {/* EXPLANATIONS LIST */}
          <div className="space-y-4">
            <h3 className="text-sm font-black text-slate-500 uppercase tracking-wider">Comprehensive Question Breakdown</h3>
            ...
            <div className="space-y-6">
              {questions.map((q, idx) => {
                ...
                return (
                  <div key={q.id} className="p-5 bg-slate-50 border border-slate-100 rounded-2xl space-y-3.5">
                    <div className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-100">
                      <div className="flex flex-col">
                        <span className="text-xs font-bold text-slate-700">Question {idx + 1} ({q.topic})</span>
                        <span className="text-[10px] text-slate-400 font-mono mt-0.5 text-left">
                          Real {q.examName || 'WAEC'} Past Question • Year {q.examYear || 2021} • Q{q.questionNumber || 12}
                        </span>
                      </div>
                      <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${isCorrect ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'}`}>
                      ...
                      <div className="bg-white p-3.5 rounded-xl border border-slate-100/50">
                        ...
                        <span className="font-bold text-slate-800">
                          {q.type === 'mcq' && q.options 
                            ? (userAns !== undefined ? `${String.fromCharCode(65 + Number(userAns))}) ${q.options[Number(userAns)]}` : 'Unanswered')
                            : (userAns || 'Unanswered')}
                        </span>
                      </div>
                      <div className="bg-white p-3.5 rounded-xl border border-slate-100/50">
                    ...
                    <div className="bg-indigo-50/50 rounded-xl p-4 border border-indigo-100/30 text-xs text-indigo-950">
                      <h5 className="font-bold text-slate-800 mb-1">Detailed WAEC Explanation:</h5>
                    ...
                    <div className="flex justify-start pt-1">
                      ...
                      >
                        <Sparkles className="w-3.5 h-3.5 text-amber-300" /> Ask AI Tutor to Explain Step-by-Step
                      </button>
                    </div>
                  </div>
                );
          ...
          <div className="bg-white rounded-3xl border border-slate-100 max-w-sm w-full p-6 text-center space-y-4 shadow-2xl">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 leading-relaxed text-center">
                Your progress in this practice or exam session will not be saved.
              </p>
            </div>
