...
function generateOneQuestion(
  ...
  // Choose topic
  const topics = {
    'Mathematics': ['Quadratic Equations', 'Trigonometry', 'Set Theory', 'Calculus (Further Maths)', 'Statistics', 'Geometry', 'Logarithms', 'Arithmetic Progression (AP)'],
    'English Language': ['Lexis and Structure', 'Prepositions', 'Synonyms & Antonyms', 'Direct & Indirect Speech', 'Reading Comprehension'],
    'Physics': ['Mechanics & Motion', 'Radioactivity', 'Electricity', 'Waves', 'Thermodynamics', 'Gravitational Fields'],
    'Chemistry': ['Gas Laws', 'Chemical Equilibrium', 'Atomic Structure', 'Electrochemistry', 'Organic Chemistry', 'Stoichiometry'],
    'Biology': ['Cell Biology', 'Genetics', 'Ecology', 'Skeletal Systems', 'Evolution', 'Plant Nutrition'],
    'Economics': ['Demand & Supply', 'Market Structures', 'National Income', 'Inflation', 'International Trade'],
    'Government': ['Constitutional Development', 'Types of Government', 'Public Administration', 'Foreign Policy']
  ...
  if (subject === 'Mathematics') {
    if (topic === 'Quadratic Equations') {
      ...
      const roots = [[2, 3], [1, 5], [2, 4], [3, 4], [1, 6], [3, 5]][seed % 6];
      const r1 = roots[0];
      const r2 = roots[1];
      const sum = r1 + r2;
      const prod = r1 * r2;
      ...
      if (angle === 30) {
        ...
        options = [`${height}m`, `${height}√3m`, `${ladderLen}m`, `${height / 2}m`];
        correctAnswer = '0';
    ...
    if (topic === 'Prepositions') {
      const optionsPool = [
        ...
        ['charged with complicity _______ the conspiracy', 'in', 'of', 'for', 'about'],
        ['protested bitter _______ the new levies', 'against', 'for', 'on', 'with'],
        ['accused her _______ stealing the school files', 'of', 'for', 'about', 'on']
      ];
      ...
      options = [selectedPool[1], selectedPool[2], selectedPool[3], selectedPool[4]];
      ...
      const optionsPool = [
        ['Neither of the two candidates _______ qualified for the senior administrator position.', 'is', 'are', 'were', 'have been', '0', 'According to concord rules, "neither" as a pronoun is singular and takes the singular verb "is".'],
        ['By the time the school head arrived, the students _______ the assembly ground.', 'had left', 'have left', 'were leaving', 'left', '0', 'When two actions occur in the past, the earlier event takes the past perfect tense ("had left").'],
        ['The teacher, as well as the students, _______ expected to attend the sports seminar.', 'is', 'are', 'have been', 'were', '0', 'When parenthetical phrases like "as well as" are used, the verb agrees with the first subject ("The teacher"), which is singular ("is").']
      ];
      const selectedPool = optionsPool[seed % optionsPool.length];
      ...
      const words = [
        ['gregarious', 'sociable', 'hostile', 'shy', 'unfriendly', 'nearest in meaning', 'extroverted, fond of company'],
        ['ephemeral', 'short-lived', 'everlasting', 'beautiful', 'expensive', 'nearest in meaning', 'lasting for a very short duration'],
        ['taciturn', 'silent', 'garrulous', 'loud', 'vocal', 'nearest in meaning', 'reserved or uncommunicative in speech'],
        ['scrupulous', 'careful', 'negligent', 'unreliable', 'dishonest', 'nearest in meaning', 'extremely attentive to details; careful']
      ];
      ...
      options = [selectedWord[1], selectedWord[2], selectedWord[3], selectedWord[4]];
    ...
    if (topic === 'Electricity') {
      ...
      if (seed % 2 === 0) {
        ...
        options = [`${r1 + r2}Ω`, `${((r1 * r2) / (r1 + r2)).toFixed(2)}Ω`, `${r1}Ω`, `${r2}Ω`];
        correctAnswer = '0';
      ...
      options = [`${lambda.toFixed(2)}m`, `${(v * f).toFixed(0)}m`, `${f / v}m`, `0.5m`];
      correctAnswer = '0';
      ...
      options = [`${ke} J`, `${mass * velocity} J`, `${ke * 2} J`, `${mass * velocity * velocity} J`];
      correctAnswer = '0';
    ...
    if (topic === 'Gas Laws') {
      ...
      options = [`${v2} cm³`, `${v1 * 2} cm³`, `${v1 * 4} cm³`, `${v2 / 2} cm³`];
      correctAnswer = '0';
      ...
      const element = [['Carbon-14', 14, 6], ['Oxygen-18', 18, 8], ['Sodium-23', 23, 11], ['Chlorine-37', 37, 17]][seed % 4];
      ...
      options = [`${neutrons}`, `${atomicNum}`, `${massNum}`, `${massNum + atomicNum}`];
      correctAnswer = '0';
      ...
      // Organic Chemistry
      const formula = ['CH3-CH2-CH3', 'CH3-CH2-CH2-CH3', 'CH3-CH=CH2', 'CH3-C≡CH'][seed % 4];
    ...
    if (topic === 'Cell Biology') {
      const organelles = [
        ['Mitochondrion', 'is responsible for respiration and generating ATP', 'chloroplast', 'ribosome', 'vacuole'],
        ['Ribosome', 'is the organelle that serves as the site for protein synthesis', 'mitochondrion', 'nucleus', 'lysosome'],
        ['Chloroplast', 'contains chlorophyll and acts as the site for photosynthesis', 'mitochondrion', 'cell wall', 'vacuole'],
        ['Lysosome', 'functions as the digestive center for destroying worn-out cell structures', 'Golgi complex', 'ribosome', 'centriole']
      ];
      ...
      options = ['Third trophic level', 'First trophic level', 'Second trophic level', 'Fourth trophic level'];
      correctAnswer = '0';
    ...
    if (topic === 'Demand & Supply') {
      ...
      options = ['Elastic', 'Inelastic', 'Unitary elastic', 'Perfect inelastic'];
      correctAnswer = '0';
      ...
      options = ['Cost-push inflation', 'Demand-pull inflation', 'Hyperinflation', 'Creeping inflation'];
      correctAnswer = '0';
    ...
    if (topic === 'Constitutional Development') {
      const constitution = [
        ['Clifford Constitution', '1922', 'introduced the elective principle in Nigeria allowing partial voting'],
        ['Richards Constitution', '1946', 'separated Nigeria into three regional councils (North, West, East)'],
        ['Macpherson Constitution', '1951', 'created central legislative houses and regional assemblies'],
        ['Lyttelton Constitution', '1954', 'established a full federal model with political governors in Nigeria']
      ];
      ...
      options = [selectedCon[0], 'Bourdillon Constitution', 'Macpherson Constitution', 'Independence Constitution'];
      ...
      options = ['A unitary system of government', 'A federal system of government', 'A loose confederation system', 'An absolute feudal structure'];
      correctAnswer = '0';
...
export function getCbtSimulationQuestions(
  ...
  // 1. Filter original dataset
  const poolToUse = customQuestionsPool || SAMPLE_QUESTIONS;
  let subjectPool = poolToUse.filter(q => q.subject.trim().toLowerCase() === subject.trim().toLowerCase());
  
  if (topicFilter && topicFilter.trim() !== '') {
    subjectPool = subjectPool.filter(q => q.topic.trim().toLowerCase() === topicFilter.trim().toLowerCase());
  }

  // 2. Separate into targeted year and other years' real questions
  let primaryPool: Question[] = [];
  let secondaryPool: Question[] = [];

  if (year !== 'all') {
    // Standardize comparison is flexible
    primaryPool = subjectPool.filter(q => q.examYear === Number(year) || q.examYear === year);
    secondaryPool = subjectPool.filter(q => q.examYear !== Number(year) && q.examYear !== year);
  } else {
  ...
  // Shuffle both lists to provide variance for simulated mix
  primaryPool = [...primaryPool].sort(() => 0.5 - Math.random());
  secondaryPool = [...secondaryPool].sort(() => 0.5 - Math.random());

  // Accumulate questions
  let chosenQuestions = [...primaryPool];

  // If we need more questions, fill with real ones from other years first
  if (chosenQuestions.length < targetCount) {
    const spaceLeft = targetCount - chosenQuestions.length;
    chosenQuestions = [...chosenQuestions, ...secondaryPool.slice(0, spaceLeft)];
  }

  // If we still need more questions (because we ran out of real database questions),
  // fill with premium procedurally generated ones
  if (chosenQuestions.length < targetCount) {
    const needed = targetCount - chosenQuestions.length;
    const synths: Question[] = [];
    const startNum = chosenQuestions.length + 1;
    ...
    chosenQuestions = [...chosenQuestions, ...synths];
  } else {
