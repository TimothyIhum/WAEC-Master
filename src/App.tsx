...
export default function App() {
  ...
  const t = (text: string): string => {
    ...
    const ITALIAN_DICTIONARY: Record<string, string> = {
      ...
      "Install the entire study suite as a standalone client to practice all subjects completely offline.":
        "Installa l'intero pacchetto di studio offline come client autonomo per esercitarti in tutte le materie.",
  ...
  const refreshQuestionsFromDatabase = React.useCallback(async () => {
    try {
      const cloudQs = await syncQuestionsFromDatabase();
      setQuestionsList(cloudQs);
      localStorage.setItem("waec_questions_list", JSON.stringify(cloudQs));
      return cloudQs;
    } catch (e) {
      console.error("Questions database sync failed:", e);
      return [];
    }
  }, []);
        ...
        try {
          ...
          if (parsed && parsed.email) {
            ...
            const profileResp = await fetch(
              `/api/users/profile?email=${encodeURIComponent(emailLower)}`,
            );
            if (profileResp.ok) {
              ...
              console.warn(
                "Active account was deleted from the database. Resetting session.",
              );
              setUser(null);
              setShowLanding(true);
          ...
          console.error(
            "Failed to re-validate user session on page reload:",
            e,
          );
        }
      }

      await refreshQuestionsFromDatabase();
    };
    initSync();
  ...
  React.useEffect(() => {
    if (user) {
      ...
      saveUserToFirestore(user).catch((e) =>
        console.error("Firestore progress sync failed:", e),
      );
      ...
      try {
        ...
        dbUser = parsed.find(
          (u: any) =>
            ...
            (emailStr &&
              u.email.toLowerCase().trim() === emailStr.toLowerCase().trim()),
        );
        if (dbUser) {
          const emailLower = (dbUser.email || "").toLowerCase().trim();
          if (
            emailLower === "timothyihum@gmail.com" ||
            emailLower === "temiokusami@gmail.com" ||
  ...
  const handleAddQuestion = async (q: Question) => {
    try {
      await saveQuestionToDatabase(q);
      await refreshQuestionsFromDatabase();
    } catch (e) {
      console.error("Cloud questions sync failed:", e);
    }
  };

  const handleDeleteQuestion = async (qId: string) => {
    try {
      await deleteQuestionFromDatabase(qId);
      await refreshQuestionsFromDatabase();
    } catch (e) {
      console.error("Question deletion sync failed:", e);
    }
  };

  const handlePostAnnouncement = (ann: {
    title: string;
    content: string;
    category: any;
  }) => {
  ...
  return (
    <div
      id="waec-master-app"
      className="min-h-screen bg-slate-50 flex flex-col font-sans selection:bg-indigo-600 selection:text-white"
    >
      ...
      {!showLanding && !user && (
        <AuthPage
          ...
          onAuthSuccess={(profile) =>
            handleAuthSuccess(profile.username, false, profile.email, profile)
          }
        ...
        <div className="flex grow relative">
          ...
          <div className="lg:hidden w-full fixed top-0 left-0 bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 z-40 text-slate-800 shadow-xs">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-extrabold text-base shadow-md shadow-indigo-100">
                W
              </div>
            ...
            <div className="lg:hidden fixed inset-0 top-16 bottom-0 bg-white/95 backdrop-blur-xs z-50 text-slate-800 p-6 flex flex-col gap-6 shadow-lg overflow-y-auto">
              <div className="space-y-6 flex-1">
                <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  <div>
                    ...
                    <span className="text-3xs text-indigo-600 font-extrabold">
                      {user.isAdmin
                        ? "Exam Admin"
                        : `Level ${user.level} Candidate`}
                    </span>
              ...
              <div className="space-y-2.5 mt-auto shrink-0 pb-6">
                <button
                  ...
                  onClick={() => {
                    setMobileMenuOpen(false);
                    downloadOfflineCbtApp(questionsList);
                  }}
            ...
            <header className="hidden lg:flex h-20 px-8 items-center justify-between bg-white border-b border-slate-200 shrink-0 sticky top-0 z-30">
              ...
              >
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-indigo-500 font-extrabold uppercase tracking-widest">
                ...
                {activeTab === "guardian" && (
                  <div className="animate-fadeIn">
                    <ParentDashboard
                      ...
                      onUpdateParentConfig={(updatedConfig) =>
